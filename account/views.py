...
def password_change(request):
    ...
    if http_method == "GET":
        form = PasswordChangeForm(request.user) # 비밀번호 변경할 user 정보를 넣어 빈폼을 생성 - 기존 패스워드 확인용
        ...
        if form.is_valid(): # 검증 통과
            # db update
            user = form.save() # ModelForm.save(): Updae/Insert 한 모델 객체
            # session에 저장된 user 정보를 변경된 내용으로 변경 (안 하면 logout)
            update_session_auth_hash(request, user) # request, 변경된 user 모델
            # 응답
...
def user_update(request):
    http_method = request.method
    if http_method == "GET":
        # CustomUserChangeForm을 이용해 빈폼을 생성 - 로그인한 User 객체를 넣어 생성. 입력 필드에 기존 데이터가 나와야 함.
        ...
        if form.is_valid(): # 검증 성공
            # DB update
            user = form.save()
            # update session of login user information

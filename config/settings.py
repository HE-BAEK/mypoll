...
#####################################################################
# static file (html에서 사용할 img, js, css, ... files) 설정
# 1. app/static 아래 저장 -> static 파일들을 자동으로 장고서버가 인식
# 2. 1 이외 경로에 static 파일을 저장 -> settng.py에 설정
#####################################################################
# static 파일을 client에서 요청할 때 사용할 url의 시작 path
...
# - 운영환경
## http 서버 + wsgi(django 실행환경) 나눠서 서버를 구축할 때
## http 서버가 static 파일 요청을 처리
## static 파일들의 경로를 http 서버에게 알려줘야 한다
# static 파일들을 한 디렉토리(STATIC_ROOT)에 모은 뒤 그 디렉토리를 알려준다.
# http 서버: apache httpd, nginx
